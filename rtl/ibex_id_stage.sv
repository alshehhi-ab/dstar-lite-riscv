...
// D* Lite custom ISA extension outputs
  output logic                      custom_insn_ex_o,
  output logic [2:0]                custom_funct3_ex_o,
  output logic [6:0]                custom_funct7_ex_o,
  ...
  assign custom_insn_ex_o    = custom_insn;
  assign custom_funct3_ex_o  = custom_funct3;
  assign custom_funct7_ex_o  = custom_funct7;
  // One-shot enable: fires only on the first executing cycle of a custom instruction.
  assign custom_en_ex_o      = custom_insn & instr_first_cycle & instr_executing;

  assign custom_insn       = (opcode_e'(instr_rdata_i[6:0]) == OPCODE_CUSTOM0);
  assign custom_funct3     = instr_rdata_i[14:12];
  assign custom_funct7     = instr_rdata_i[31:25];
  // Multi-cycle custom: everything except HEUR (funct3=111) needs memory access
  assign custom_multicycle = custom_insn & (instr_rdata_i[14:12] != 3'b111);
    ...
    if (instr_executing_spec) begin
      unique case (id_fsm_q)
        ...
        MULTI_CYCLE: begin
          ...
          if (custom_multicycle) begin
            rf_we_raw = rf_we_dec & ex_valid_i;
            if (~ex_valid_i) stall_alu = 1'b1;
          end
